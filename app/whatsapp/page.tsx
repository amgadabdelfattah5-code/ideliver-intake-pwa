  ...
  return <main className="min-h-screen bg-[#f6f8fb] text-[#17365F]">
    ...
    <section className="mx-auto max-w-6xl px-4 py-6">
      ...
      {tab === 'logs' && <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="overflow-x-auto" dir="ltr"><table className="min-w-max text-left text-sm"><thead className="bg-slate-50"><tr>{['Date','Order','Recipient','Message','Status','Response','Action'].map((heading) => <th className="px-2 py-2 whitespace-nowrap" key={heading}>{heading}</th>)}</tr></thead><tbody>{logs.map((log) => <tr className="border-t border-slate-100" key={log.id}><td className="px-2 py-2 whitespace-nowrap">{log.date}</td><td className="px-2 py-2 whitespace-nowrap">#{log.order_id}</td><td className="px-2 py-2 whitespace-nowrap">{log.recipient_number}</td><td className="px-2 py-2 max-w-[220px] truncate" title={log.message_content}>{log.message_content}</td><td className={`px-2 py-2 whitespace-nowrap font-semibold ${log.status === 'success' ? 'text-green-700' : 'text-red-700'}`}>{log.status}</td><td className="px-2 py-2 max-w-[180px] truncate" title={log.api_response}>{log.api_response}</td><td className="px-2 py-2 whitespace-nowrap">{log.status === 'failed' && <button className="idv-button idv-button-small" disabled={resending === log.id} onClick={() => resend(log)}>{resending === log.id ? 'Sending...' : 'Resend'}</button>}</td></tr>)}</tbody></table></div>
